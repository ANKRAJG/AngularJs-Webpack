(function () {
    ...
    ExportController.$inject = ['$log', '$http', '$location', 'AuthenticationService', 'FlashService', '$uibModal', 'MAINURL', 'APIURL','ImageURL','$scope','$rootScope','SmartRegisterService', '$timeout','$window', 'EventService', 'RememberService', 'ModalService', '$routeParams'];
    function ExportController($log, $http, $location, AuthenticationService, FlashService, $uibModal,MAINURL,APIURL,ImageURL,$scope, $rootScope, SmartRegisterService, $timeout, $window, EventService, RememberService, ModalService, $routeParams) {
        var vm = this;
        ...
        function DownloadFormatExcel() {
        	vm.gridName = "exportSubmittalGrid";
        	GetFormatSpecificFile(vm.exportId, vm.gridName);
        }
		 ...
		 function initializeQuickFilterVaraiables(){
          ...
          Custom filter for submittal type
	        $scope.submittalTypeFilter = function(item){
	        	if(vm.searchSubmittalType == undefined || vm.searchSubmittalType == '' || (item.submittalType && item.submittalType.toUpperCase().indexOf(vm.searchSubmittalType.toUpperCase()) > -1)
	        	          || (item.submittalType && item.submittalType.indexOf(vm.searchSubmittalType) > -1))
	        	          return true;
	        	         return false;

	        }

	        $scope.showMoreSpecSections = function(sectionsLimit, maxSections){
	        	$scope.sectionsLimit = increaseFilterSections(sectionsLimit, maxSections, $scope.filterSectionsRange);
	        }

	        $scope.showLessSpecSections = function(sectionsLimit, maxSections){
	        	$scope.sectionsLimit = reduceFilterSections(sectionsLimit, maxSections, $scope.filterSectionsRange);
	        }

	        $scope.showMoreDivisionCodes = function(sectionsLimit, maxSections){
	        	$scope.divisionsLimit = increaseFilterSections(sectionsLimit, maxSections, $scope.filterDivisionsRange);
	        }

	        $scope.showLessDivisionCodes = function(sectionsLimit, maxSections){
	        	$scope.divisionsLimit = reduceFilterSections(sectionsLimit, maxSections, $scope.filterDivisionsRange);
	        }
     ...
     function getUniqueSubmittalTypes(submittals) {
        ...
        function addToFilterClicked(){
        	...
        	/*if(vm.filterName !== vm.currentFilter.name){
        		vm.currentPreferenceId = -1;
        	}*/
